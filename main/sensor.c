 ...
 */
esp_err_t send_message(float temperature, float humidity) {
	...
	esp_err_t err = ubirch_send(CONFIG_UBIRCH_BACKEND_DATA_URL, sbuf->data, sbuf->size, unpacker);
	if (err != ESP_OK) {
		ESP_LOGE(__func__, "unable to send data");
	}
...
/*!
 * The main sensor measurement function.
 * This function gives a Semaphore back, which other functions can wait for.
 */
void sensor_loop_float(float *f_hall, float *f_temp) {
	// let the LED blink
	if (interval < 1000) gpio_set_level(BLUE_LED, 0); else gpio_set_level(BLUE_LED, 1);

	*f_hall = (float) hallRead();
	*f_temp = temperatureReadFixed();
	ESP_LOGI(__func__, "Hall Sensor = %f", *f_hall);
	ESP_LOGI(__func__, "Temp Sensor = %f", *f_temp);
}
